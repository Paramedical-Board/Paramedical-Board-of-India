...
export default function AffiliatedInstitutionsClient({ institutions }: { institutions: AffiliatedInstitution[] }) {
  ...
  const states = useMemo(() => {
    const list = Array.from(new Set(institutions.map((i) => i.state)));
    return ["All States", ...list];
  }, []);

  const districts = useMemo(() => {
    const list = Array.from(new Set(institutions.map((i) => i.district)));
    ...
    <div className="min-h-screen bg-[#F8FAFC] flex flex-col font-sans text-slate-800 selection:bg-[#143E66] selection:text-white">
      ...
      <main className="max-w-[1440px] mx-auto w-full px-4 sm:px-6 lg:px-8 py-8 sm:py-10 flex-1">
        ...
        <div className="flex items-center justify-between mb-6 text-xs sm:text-sm text-slate-600">
          <div>
            Showing <strong className="text-[#143E66]">{filteredInstitutions.length}</strong> of {institutions.length} Affiliated Institutions
          </div>
